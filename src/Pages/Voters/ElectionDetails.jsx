...
const ElectionDetails = () => {
  ...
  // Debugging - log when vote status changes
  useEffect(() => {
    console.log("Vote status updated:", pageState.hasVoted);
    console.log("Vote summary:", pageState.voteSummary);
  }, [pageState.hasVoted, pageState.voteSummary]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (publicResponse.positions && Array.isArray(publicResponse.positions)) {
          ...
          if (publicResponse.candidates && Array.isArray(publicResponse.candidates)) {
        ...
        const formattedElectionData = {
          ...
          title: publicResponse.election.election_name || publicResponse.election.name,
          department: publicResponse.election.department || 'Department Election',
        ...
        if (user && token) {
          try {
            ...
            const voteResponse = await getElectionDetails(token, `${id}?t=${timestamp}`);
            
            console.log("Vote response received:", voteResponse);
            ...
            if (isMounted) {
              setPageState(prev => {
                ...
                if (voteResponse && typeof voteResponse === 'object') {
                  ...
                  if (updatedState.hasVoted && 
                      voteResponse.userVoteDetails && 
                      Array.isArray(voteResponse.userVoteDetails)) {
                    updatedState.voteSummary = voteResponse.userVoteDetails.filter(detail => 
  ...
  const handleVoteNow = () => {
    ...
    if (!isElectionOpen) {
      setPageState(prev => ({ 
        ...prev, 
        voteError: 'Voting is closed. This election is not currently open for voting.' 
      }));
      return;
    }
